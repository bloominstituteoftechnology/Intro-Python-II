            ...
            if (num_args[0] == "take" or num_args[0] == "get"):
                ...
                for count, item in enumerate(current_player.room.items):
                    if (num_args[1] == item.name):
                        item_present = True
                        current_player.inventory.append(current_player.room.items[count])
                        current_player.room.items[count].on_take()
                        current_player.room.items.remove(current_player.room.items[count])
                    else:
                        pass
                if (item_present):
                    pass
                ...
                for count, item in enumerate(current_player.inventory):
                    if (num_args[1] == item.name):
                        item_present = True
                        current_player.room.items.append(current_player.inventory[count])
                        current_player.inventory[count].on_drop()
                        current_player.inventory.remove(current_player.inventory[count])
                    else:
                        pass
                if (item_present):
                    pass
