...
from item.field import Field
from item.field import ItemListField
from item.error import ItemRuntimeError

logger = logging.getLogger('item.item')
...
class Item(ItemBuilderMetaClass):
    ...
    def _build_selector(cls, tree, selector_type):
        if selector_type == 'xpath':
            ...
            raise ItemRuntimeError('Unknown selector type: %s' % selector_type)
    ...
    def extract_document_data(cls, grab):
        ...
        if sel_type == 'xpath':
            ...
            raise ItemRuntimeError('Unknown selector type: %s' % sel_type)
